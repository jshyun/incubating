...
public class CERTRecordTest extends AbstractRecordTest {

    @Test 
    public void testBasic() {

        final CertificateType type = CertificateType.IPKIX;
        final short keyTag = 2;
        final Algorithm algorithm = Algorithm.RSASHA1;
        final ByteString certificate = ByteString.from(4, 5, 6, 7, 8, 9);
        
        CERTRecord r = new CERTRecord(STD_NAME, STD_CLASS, STD_TTL, type, keyTag, algorithm,
                certificate);
        assertBaseRecord(STD_NAME, STD_CLASS, STD_TTL, r);

        Assert.assertEquals(type, r.getType());
